...
x_grid <- seq(-4, 4, length.out = 800)
y_grid <- seq(-5.5, 5.5, length.out = 800)
...
y_grid <- seq(-5.5, 5.5, length.out = 100)
...
d <- ncol(samples)
boundaries <- matrix(c(-4,-5.5,4,5.5),nrow = 2)
...
sum(base_measure_weights)
### Check base measure
x1 <- samples[, 1]
x2 <- samples[, 2]

# Compute Voronoi diagram
vor <- deldir(x1, x2, rw = c(-4, 4, -5.5, 5.5))
tiles <- tile.list(vor)
summary_df <- vor$summary

# Start empty plot
plot(0, 0, type = "n", xlim = c(-4, 4), ylim = c(-5.5, 5.5), asp = 1,
...
density_at_samples <- function(centered_kernel_mat_samples, lambda, weight_hat_vec) {
  ...
  p_vec <- sapply(1:nrow(centered_kernel_mat_samples), function(i) {
    ...
    exp(lambda * (weight_hat_vec %*% centered_kernel_mat_samples[,i] -
                        0.5 * diag_vals[i]))
  })
...
density_at_grids <- function(centered_kernel_mat_grids,
  ...
  p_vec <- sapply(1:ncol(centered_kernel_mat_grids), function(i) {
    ...
    exp(lambda * (weight_hat_vec %*% centered_kernel_mat_grids[,i] -
...
get_dens_or_prob <- function(centered_kernel_mat_samples,
  ...
  dens_samples <- density_at_samples(centered_kernel_mat_samples, lambda, weight_hat_vec)
  ...
  #cat(dim(dens_samples))
  #cat(length(dens_samples))

  #cat(dim(base_measure_weights))
  #cat(length(base_measure_weights))
...
# Define the root function: the gradient (s)
root_function <- function(weight_hat_vec) {
  ...
  s <- lambda * (
    colSums(centered_kernel_mat_samples) -
      nrow(centered_kernel_mat_samples) * as.numeric(prob_samples %*% t(centered_kernel_mat_samples))
  ) - tau * weight_hat_vec / prob_samples

  return(as.vector(s))  # return as vector for BBsolve
}
...
as.numeric(probs$samples %*% t(centered_kernel_mat_samples))


colSums(centered_kernel_mat_samples)

weights_package <- get_weights(lambda,
                        tau,
                        centered_kernel_mat_samples,
                        samples,
                        base_measure_weights,
...
initial_guess <- rep(0, nrow(centered_kernel_mat_samples))

# Solve using BBsolve
solution <- BBsolve(par = initial_guess, fn = root_function,
                    control = list(maxit = 10000,
                                   tol = 1e-4,
                                   trace = TRUE))

# Extract the estimated weights
weight_hat_vec <- solution$par

base_measure_weights

as.numeric(root_function(weight_hat_vec))

as.numeric(get_s_function(weight_hat_vec,
               lambda,
               tau,
               centered_kernel_mat_samples,
               samples,
...
# Print final result
cat("Converged:", solution$convergence == 0, "\n")
cat("Final ||s||_2 norm:", norm(root_function(weight_hat_vec), type = "2"), "\n")

d1 <- get_dens_wo_grid(centered_kernel_mat_samples,samples,base_measure_weights,
                       dimension = 2,lambda = lambda,weight_vec = weight_hat_vec)

d2 <- get_dens_or_prob(centered_kernel_mat_samples,samples,base_measure_weights,
                       lambda = lambda,type_of_p_is_prob = F,
                       weight_hat_vec = weight_hat_vec)
...
d4_wbase <- get_dens_or_prob(centered_kernel_mat_samples,samples,base_measure_weights,
                       lambda = lambda,type_of_p_is_prob = T,
                       weight_hat_vec = weight_hat_vec)$samples

d3_wbase <- as.numeric(d1 * base_measure_weights)
...
get_dens_or_prob_just_for_output <- function(centered_kernel_mat_samples,
  ...
  print("***")
  # Compute the densities at the grid points
  dens_grids <- density_at_grids(centered_kernel_mat_grids, centered_kernel_self_grids, lambda, weight_hat_vec)
...
# Compute true density on the grid
library(mvtnorm)
true_density <- 0.6 * dmvnorm(grids, mean = mu1, sigma = Sigma1) +
  0.4 * dmvnorm(grids, mean = mu2, sigma = Sigma2)

grid_df <- as.data.frame(grid_mat)
grid_df$true_density <- true_density
colnames(grid_df) <- c("x","y", "true_density")

# Use contour plot
# === Plot estimated + true density contour ===
p_contour <- ggplot(df, aes(x = x, y = y, z = weight)) +
  ...
  # Overlay true density contours in black
  geom_contour(data = grid_df, aes(x = x, y = y, z = true_density),
               color = "black", size = 0.7, bins = 10) +
  coord_fixed() +
  theme_bw() +
  labs(
    title = "Contour Plot of Density Estimate",
    subtitle = paste("n =", n_total, ",", nrow(df), "grid points, weight grid points = 2500"),
    x = "X",
    y = "Y"
  ) +
  theme(
...
ggplot() +
  ...
  theme_bw() +
  labs(
    title = "KEF estimation lambda = 1, tau = 1/1350",
